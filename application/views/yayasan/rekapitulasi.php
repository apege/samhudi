...
if (!function_exists('render_cards_pagination')) {
    function render_cards_pagination($total_rows, $limit, $current_page, $param_name, $is_all = false) {
        ...
        $html = '<div class="flex items-center justify-center gap-2 mt-6 overflow-x-auto pb-1">';
        $html .= $toggle_btn;

        if ($current_page > 1) {
            $html .= '<a href="' . $url_prefix . ($current_page - 1) . '" class="px-3.5 py-2 rounded-xl bg-[#172e2b] hover:bg-emerald-600/40 text-white text-xs font-semibold border border-emerald-500/30 transition-all shrink-0"><i class="bi bi-chevron-left"></i></a>';
        }
        ...
        if ($start_p > 1) {
            $html .= '<a href="' . $url_prefix . '1" class="px-3 py-2 rounded-xl bg-[#172e2b] hover:bg-emerald-600/40 text-white text-xs font-semibold border border-emerald-500/30 transition-all shrink-0">1</a>';
            if ($start_p > 2) {
        ...
        for ($i = $start_p; $i <= $end_p; $i++) {
            if ($i === $current_page) {
                $html .= '<span class="px-3.5 py-2 rounded-xl bg-emerald-500 text-teal-950 text-xs font-extrabold border border-emerald-400 shadow-md shrink-0">' . $i . '</span>';
            } else {
                $html .= '<a href="' . $url_prefix . $i . '" class="px-3.5 py-2 rounded-xl bg-[#172e2b] hover:bg-emerald-600/40 text-white text-xs font-semibold border border-emerald-500/30 transition-all shrink-0">' . $i . '</a>';
            }
        }
        ...
        if ($end_p < $total_pages) {
            ...
            $html .= '<a href="' . $url_prefix . $total_pages . '" class="px-3 py-2 rounded-xl bg-[#172e2b] hover:bg-emerald-600/40 text-white text-xs font-semibold border border-emerald-500/30 transition-all shrink-0">' . $total_pages . '</a>';
        }

        if ($current_page < $total_pages) {
            $html .= '<a href="' . $url_prefix . ($current_page + 1) . '" class="px-3.5 py-2 rounded-xl bg-[#172e2b] hover:bg-emerald-600/40 text-white text-xs font-semibold border border-emerald-500/30 transition-all shrink-0"><i class="bi bi-chevron-right"></i></a>';
        }
...
<body class="bg-[#0b1413] text-white font-body min-h-screen pb-16">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-12">
        ...
        <!-- RINGKASAN TOTAL SUARA MASUK (SUMMARY STAT CARDS) -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-gradient-to-br from-[#182a27] to-[#0f1d1b] border border-amber-500/30 rounded-3xl p-5 sm:p-6 shadow-2xl flex items-center gap-4">
                <div class="w-13 h-13 rounded-2xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center shrink-0 p-3">
                    <i class="bi bi-inbox-fill text-amber-400 text-2xl"></i>
                </div>
                <div>
                    <span class="text-xs text-white/50 font-medium uppercase tracking-wider block">Total Suara Masuk</span>
                    <div class="text-2xl sm:text-3xl font-black text-amber-300 mt-0.5"><?= number_format($total_suara_masuk ?? 0) ?> <span class="text-xs font-semibold text-white/60">Suara</span></div>
                </div>
            </div>

            <div class="bg-gradient-to-br from-[#182a27] to-[#0f1d1b] border border-emerald-500/30 rounded-3xl p-5 sm:p-6 shadow-2xl flex items-center gap-4">
                <div class="w-13 h-13 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center shrink-0 p-3">
                    <i class="bi bi-person-fill text-emerald-400 text-2xl"></i>
                </div>
                <div>
                    <span class="text-xs text-white/50 font-medium uppercase tracking-wider block">Jumlah Suara Masuk Kategori Individu</span>
                    <div class="text-2xl sm:text-3xl font-black text-emerald-300 mt-0.5"><?= number_format($total_suara_individu ?? 0) ?> <span class="text-xs font-semibold text-white/60">Suara</span></div>
                </div>
            </div>

            <div class="bg-gradient-to-br from-[#182a27] to-[#0f1d1b] border border-cyan-500/30 rounded-3xl p-5 sm:p-6 shadow-2xl flex items-center gap-4">
                <div class="w-13 h-13 rounded-2xl bg-cyan-500/15 border border-cyan-500/30 flex items-center justify-center shrink-0 p-3">
                    <i class="bi bi-people-fill text-cyan-400 text-2xl"></i>
                </div>
                <div>
                    <span class="text-xs text-white/50 font-medium uppercase tracking-wider block">Jumlah Suara Masuk Kategori Rundayan</span>
                    <div class="text-2xl sm:text-3xl font-black text-cyan-300 mt-0.5"><?= number_format($total_suara_rundayan ?? 0) ?> <span class="text-xs font-semibold text-white/60">Suara</span></div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function getDistinctColorsForData(dataSeries) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                // Keep saturation extremely high (90%-100%) and lightness bright (52%-62%) for maximum neon contrast against dark background
        ...
        function render3DPieChart(dataSeries, titleText) {
            ...
            highChartInstance = Highcharts.chart('container_chart_3d', {
                ...
                legend: {
                    ...
                    labelFormat: isMobile ? '<b>{name}</b> ({roles}): {y} suara ({percentage:.0f}%)' : '{name} <span style="color:#D4B571;font-weight:bold;">({roles})</span>',
        ...
        function showRundayanModal(ancName) {
            ...
            combinedNominators = [...new Set(combinedNominators)];
            combinedCandidates = [...new Set(combinedCandidates)];
            const totalVotes = combinedNominators.length;
            ...
            if (totalVotes > 0 || combinedCandidates.length > 0) {
                ...
                document.getElementById('modal_rundayan_candidates').innerText = combinedCandidates.join(', ') || '-';
            } else {
                ...
                document.getElementById('modal_rundayan_candidates').innerText = `-`;
            }
