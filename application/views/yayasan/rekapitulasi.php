...
<body class="bg-[#0b1413] text-white font-body min-h-screen pb-16">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-12">
        ...
        ?>
        <div class="bg-gradient-to-br from-[#182a27] to-[#0f1d1b] border border-teal-700/40 rounded-3xl p-5 sm:p-6 shadow-2xl space-y-4">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-teal-700/30 pb-3">
                <div>
                    <h4 class="font-display font-bold text-base text-white flex items-center gap-2">
                        <i class="bi bi-card-checklist text-amber-400 text-lg"></i> Status Input 14 Rundayan Samhudi
                    </h4>
                    <p class="text-xs text-white/50">Daftar kelengkapan inputan suara khusus 14 Rundayan Keluarga Besar Samhudi.</p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2.5">
                <?php if (!empty($rundayan_input_status)): ?>
                    <?php foreach ($rundayan_input_status as $r): ?>
                        ...
                        ?>
                        <div class="relative group p-2.5 rounded-2xl border flex flex-col justify-between transition-all <?= $r['has_input'] ? 'bg-emerald-950/40 border-emerald-500/30 text-emerald-200 hover:border-emerald-400/60' : 'bg-rose-950/30 border-rose-500/20 text-rose-300/80 hover:border-rose-400/50' ?>">
                            <div>
        ...
        <div class="bg-gradient-to-b from-[#162724] to-[#101c1a] border border-teal-700/40 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            <div class="relative min-h-[400px] flex flex-col items-center justify-center w-full">
    ...
    <script>
        ...
        function openRundayanModal(rundayanName) {
            ...
            // Exact key lookup (keyed by master rundayan name di PHP)
        ...
        function render3DPieChartFS(dataSeries, titleText) {
            ...
            fsChartInstance = Highcharts.chart('container_chart_3d_fs', {
                ...
                legend: {
                    enabled: true,
                    labelFormat: isMobile ? '<b>{name}</b> ({primary_role}): {y} suara ({percentage:.0f}%)' : '{name} <span style="color:#D4B571;font-weight:bold;">({primary_role})</span>',
